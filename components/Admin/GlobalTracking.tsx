...
const GlobalTracking: React.FC<GlobalTrackingProps> = ({ club, sessions, students, progress, setProgress, classes, onOpenChat }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-8 rounded-[2.5rem] shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h3 className="text-2xl font-black text-gray-900 uppercase tracking-tighter">Carte de Progression</h3>
          <p className="text-xs text-gray-400 font-bold">Cliquer sur le nom d'un jeune pour lui envoyer un message.</p>
        </div>
        <select value={selectedClassId} onChange={(e) => setSelectedClassId(e.target.value)} className="bg-gray-50 border-2 border-gray-100 rounded-2xl px-6 py-3 font-black text-[10px] uppercase outline-none">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#002366] text-white">
                <th className="sticky left-0 z-40 bg-[#002366] px-8 py-6 text-left font-black uppercase text-[10px] tracking-widest border-r border-white/10 w-64">Sujets MJA</th>
                {classStudents.map(student => (
                  <th key={student.id} className="px-2 py-6 border-r border-white/10 w-12 relative group">
                    <button onClick={() => onOpenChat(student.id)} className="vertical-text-content hover:text-yellow-400 transition-colors">
                      <span className="text-[9px] font-black uppercase tracking-tighter whitespace-nowrap block rotate-180" style={{ writingMode: 'vertical-rl' }}>
                        {student.fullName}
                      </span>
                    </button>
                  </th>
              ...
              {classSessions.map(session => (
                <React.Fragment key={session.id}>
                  <tr className="bg-yellow-50/40">
                    <td className="sticky left-0 z-30 bg-yellow-50/80 backdrop-blur-md font-black text-yellow-800 px-8 py-3 text-[10px] uppercase border-r border-yellow-100">Semaine {session.number}</td>
                  ...
                  {session.subjects.map(subject => (
                    <tr key={subject.id} className="hover:bg-blue-50/10 transition-colors">
                      <td className="sticky left-0 z-30 bg-white px-8 py-4 border-r border-gray-100">
                        <p className="text-xs font-black text-gray-800">{subject.name}</p>
                        <p className="text-[9px] text-gray-400 italic mt-0.5">{subject.prerequisite}</p>
                      </td>
                      {classStudents.map(student => (
                        <td key={student.id} className="p-0 border-r border-gray-100 text-center">
                          <label className="flex items-center justify-center w-full h-full p-2 cursor-pointer hover:bg-blue-50/50">
                            <input type="checkbox" checked={isSubjectCompleted(student.id, session.id, subject.id)} onChange={() => handleToggleSubject(student.id, session.id, subject.id)} className="w-4 h-4 rounded border-gray-300 text-blue-600" />
                          </label>
                        </td>
                      ))}
                    </tr>
